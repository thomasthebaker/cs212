...
def more_pour_problem(capacities, goal, start=None):
    ...
    # your code here
    start = tuple([0 for cap in capacities])
    def isGoal(state): return goal in state

    def glassSuccessors(state):
        """Return the successors"""
        indices = range(len(state))
        result = {}
        for i in indices:
            result[update(state, i, capacities[i])] = ('fill', i)
            result[update(state, i, 0)] = ('empty', i)
            for j in indices:
                if i!=j:
                    amount = min(state[i], (capacities[j]-state[j]))
                    temp = update(state, i, state[i]-amount)
                    result[update(temp, j, state[j]+amount)] = ('pour',i,j)
        #print (result,'\n')
        return result
    return shortest_path_search(start, glassSuccessors, isGoal)
