...
the function, and the str of the formula on the .__name__ attribute.'"""
    if coefs in CACHE: return CACHE.get(coefs)
    def theFunction(x):
        return sum(coefs[i] * x**i for i in range(len(coefs)))
    theFunction.coefs = coefs
    theFunction.__name__ = ' + '.join([term(coefs[i], i) for i in range(len(coefs)-1,-1,-1) if coefs[i]!=0])
    print theFunction.__name__
    return CACHE.setdefault(coefs, theFunction)
        
def term(coef, pow):
    c = str(coef) if coef>1 else ''
    p = 'x**'+str(pow) if pow>1 else 'x'
    if pow == 0: p = ''
    if not p and not c: c='1'
    return c +' * '+ p if c and p else c+p
...
def add(p1, p2):
    ...
    print 'add('+ p1.__name__ +', '+p2.__name__+') --->' +str(newCoefs)
